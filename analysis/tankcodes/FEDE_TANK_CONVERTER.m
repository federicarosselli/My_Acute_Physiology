...
FOLDER_FROM='C:\TDT\OpenEx\Tanks\Fede_Acute_Recording_18_3_2013';

for BLOCK_NUM=BLOCK_NUM_SET
BCODE1=[];
TBCOD1=[];
ss=0;
FOLDER_TO=['\\zocconasphys1.cns.sissa.it\chronic_inv_rec\Tanks\Fede_Acute_Recording_18_3_2013\ANALYSED\Block-',num2str(BLOCK_NUM)];
...
en_p=EN_PO(TANLG(EN_PO)>(TANLG(ST_PO(n1))+0.1) & TANLG(EN_PO)<(TANLG(ST_PO(n1))+2.5));
...
F12=figure(12);
plot(TANLG(TANLG>300 & TANLG<500),YANLG(TANLG>300 & TANLG<500))
...
clearvars -except Yeneu Teneu TIMES pYeneu pTeneu pTIMES BLOCK_NUM BLOCK_NUM_SET TIME_S TIME_E PER sub_block M SPIKES ss channel BCODE TBCOD TIME_CU BLOCK_NUM FOLDER_FROM FILE_TO FOLDER_TO BLOCK_NUM_SET sub_block TEND F11 F12 F13
...
save([FOLDER_TO '\MATLAB_DATA.mat'])
