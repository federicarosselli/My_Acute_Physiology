...
%   Author: davide
%
%   This function plots the preformance and the sensory threshold of the
%   rat during the training with the staircase. It loads .mat files with
%   the relevant behavioral parameters extracted using either
%   ExtractRatBehavData_ppc or ExtractRatBehavData and saved in data files
%   with names like: name_extr.mat
%
%   _v2: It deals also with the staircase across ROTATIONS
%
%   Example:
%
%       >> Bub_PlotStairCaseTrainingData('r_A11_b098_extr.mat', 0)
%
% ???????????????????????????????????????????????????????????????????
   


function [Size, Pos, Rot, RotDep] = CHRONIC_PlotStairCaseTrainingData( FileName, FlagSave )
...
%         Size_default_for_StairRotation_InPlane = 26;    
%     end;
...
%         errordlg('WARNING: some of the default stimulus properties was changed during the experiment: ABORT analysis!');
%         return;
%     end;
% else
    % Default values of stimulus properties
...
% set(gca, 'ylim', [0 1.2], 'xlim', [5 31], 'xtick', [10:8:26]);                     %% 'ylim', [0 1], 'XDir', 'reverse');
% xlabel('Size');
...
% x_lim = get(gca, 'xlim');
% plot( x_lim, [0.5 0.5], '--k' );
% set(gca, 'ylim', [0 1.2], 'xlim', [-14 14], 'xtick', [-12:6:12]);                                                   %%gca, 'ylim', [0 1.2] );
...
% x_lim = get(gca, 'xlim');
% plot( x_lim, [0.5 0.5], '--k' );
% set(gca, 'ylim', [0 1.2], 'xlim', [-50 50], 'xtick', [-45:18:45]);                                %%gca, 'ylim', [0 1.2] );
